...
(function ($) {
    ...
    var $currentThemeStyle = ($dtStyle) ? $dtStyle : 'style-6';
...
function changeLayout($layout, $init) {
    ...
    if (!$init)
        notifyUser('Diseño actualizado con éxito.');
}
...
function changeTheme($theme, $style, $themeStylesheet, $init) {
    ...
    if ($theme === 'lite') {
        $logo.attr('src', 'assets/images/main.png');
        ...
        $body.find('.dt-header .dt-brand__logo-img').attr('src', 'assets/images/main.png');
        $body.find('.dt-login__content-inner .dt-brand__logo-img').attr('src', 'assets/images/main.png');
    ...
    if (!$init)
        notifyUser('Tema Actualizado con éxito.');
}
